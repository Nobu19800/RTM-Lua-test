...
-- This module's spesification
-- <rtc-template block="module_spec">
local measure_luajit_spec = {["implementation_id"]="measure_luajit",
		 ["type_name"]="measure_luajit",
...
-- @class measure_luajit
-- @brief measurement of the processing time
--
-- 処理時間計測用RTC
local measure_luajit = {}
measure_luajit.new = function(manager)
	local obj = {}
	...
	function obj:onActivated(ec_id)
		...
		for i = 1, 10000 do
			...
			chr.pos_x = math.random(-10000, 10000)
			chr.pos_y = math.random(-10000, 10000)
			self.chrs[tostring(i)] = chr
		end
		
		
		self.write_file = io.open("luajit_test.dat", "a")
...
measure_luajit.Init = function(manager)
    local profile = openrtm.Properties.new{defaults_map=measure_luajit_spec}
...
local MyModuleInit = function(manager)
    measure_luajit.Init(manager)

    -- Create a component
    local comp = manager:createComponent("measure_luajit")
end
